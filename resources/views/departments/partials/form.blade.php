<div class="row">
  <div class="col-md-6 mb-4">
    <label for="sector_id" class="form-label">{{ __('Sector') }} <span class="text-danger">*</span></label>
  ...
  <div class="col-md-6 mb-4">
    <label for="code" class="form-label">{{ __('Department Code') }} <span class="text-danger">*</span></label>
    <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code"
      value="{{ old('code', $department->code ?? '') }}" placeholder="{{ __('DEPT-001') }}" required>
    @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
  </div>

  <div class="col-md-6 mb-4">
    <label for="name" class="form-label">{{ __('Name (English)') }} <span class="text-danger">*</span></label>
  ...
  <div class="col-md-6 mb-4">
    <label for="manager_id" class="form-label">{{ __('Department Manager') }}</label>
  ...
  <div class="col-md-6 mb-4">
    <label for="general_manager_id" class="form-label">{{ __('General Manager') }}</label>
    <select class="form-select select2 @error('general_manager_id') is-invalid @enderror" id="general_manager_id" name="general_manager_id">
      <option value="">{{ __('Select General Manager') }}</option>
      @foreach($managers as $manager)
    ...
    <small class="form-text text-muted">{{ __('General Manager will be CC\'d on notifications') }}</small>
  </div>
  ...
  <div class="col-md-6 mb-4">
    ...
    <select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active" required>
      <option value="1" {{ old('is_active', $department->is_active ?? '1') == '1' ? 'selected' : '' }}>{{ __('Active') }}</option>
      <option value="0" {{ old('is_active', $department->is_active ?? '1') == '0' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
    </select>
...
<div class="pt-4">
  ...
  <a href="{{ route('departments.index') }}" class="btn btn-label-secondary">{{ __('Cancel') }}</a>
</div>

@push('scripts')
<script>
  $(document).ready(function() {
    $('.select2').select2({ theme: 'bootstrap-5', placeholder: '{{ __("Select option") }}', allowClear: true });
  });
</script>
@endpush
